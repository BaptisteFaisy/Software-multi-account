    ...
    pub fn new(size: COORD, input: FileDescriptor, output: FileDescriptor) -> Result<Self, Error> {
        ...
        let result = unsafe {
            (CONPTY.CreatePseudoConsole)(
                ...
                // INHERIT_CURSOR makes ConPTY emit ESC[6n and wait for a
                // cursor-position reply.  A headless server starts reading
                // before the browser WebSocket subscribes, so that request is
                // lost and the shell remains blocked waiting for the reply.
